...
class SNN(torch.nn.Module):
    def __init__(self, rf, cf, mclk_freq, vth, std_gain=None):
        super(SNN, self).__init__()
        self.fc1 = neurons.opamp.Opamp_Dense(4, 16, rf, cf, mclk_freq, vth, std_gain)
...
if __name__ == '__main__':
    ...
    model = SNN(rf=50e3, cf=1e-12, mclk_freq=100e6, vth=-0.1, std_gain=60)
    ...
    for epoch in pbar:
        ...
        loss = criterion(outputs, train_labels) + 10 * spike_count / TIMESTEPS
        loss.backward()
        optimizer.step()
        scheduler.step()
        ...
        with torch.no_grad():
            ...
            if loss <= best_loss:
                best_loss = loss
                torch.save(model.state_dict(), 'iris_model_opamp.pth')

        pbar.set_description(f'Loss: {loss.item():.4f}')
        pbar.set_postfix(Accuracy=f'{accuracy * 100:.2f}%', Best_Loss=f'{best_loss:.4f}')

    model.load_state_dict(torch.load('iris_model_opamp.pth', weights_only=True))
